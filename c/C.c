...
static void remove_dir(char *path)
{
	...
	while ((e = readdir(d)) != NULL) {
		if (strcmp(e->d_name, ".") != 0
		    && strcmp(e->d_name, "..") != 0) {
			file =
			    str_concat(str_concat(str_dup(path), "/"),
				       e->d_name);
			unlink(file);
			free(file);
		}
	...
	if ((d = opendir(cache_root)) != NULL) {
		while ((e = readdir(d)) != NULL) {
			...
			if (stat(filename, &st) == 0
			    && S_ISDIR(st.st_mode)) {
				if (oldest_dir == NULL
				    || st.st_mtime < oldest_mtime) {
					free(oldest_dir);
					oldest_dir = str_dup(filename);
					oldest_mtime = st.st_mtime;
				}
				if (++num_files >= FILES_PER_CACHEDIR
				    && oldest_dir != NULL) {
...
static char **sa_concat(char **x, char *y)
{
	...
	if ((x = realloc(x, sizeof(char *) * (len + 2))) == NULL) {	//一般用这种方式扩充空间，注意是+2
		cmd_error("out of memory\n");
	}
	x[len++] = y;
	x[len++] = NULL;	//这里的++是无所谓的，可以不要
	return x;
}
...
static char **split_tokens(char *line)
{
	...
	while (1) {
		...
		for (line++; *line != '\0' && !isspace((int) *line);
		     line++);
		if (*line == '\0') {
			break;
		}
...
{
	while (*argv != NULL && argv[0][0] == '-') {	//argv[0]
		char *arg = *argv++;
		if (strcmp(arg, "-") == 0) {
			argv--;
			break;
			...
			if (arg[2] == '\0') {
				if (*argv == NULL) {
					cmd_error
					    ("%s not followed by a GCC argument\n",
					     arg);
				}
			...
			if (*argv == NULL) {
				cmd_error
				    ("-e should be followed by an expression\n");
			}
			if (oneliner) {
				cmd_error
				    ("multiple -e options not permitted.\n");
			}
...
int main(int argc, char **argv)
{
	...
	argv++;			//argv[0]丢掉
	{			/* parse args, determine cache dir *///这种直接用大括号括起来是为了增加可读性
	...
	if (use_main) {
		...
		gcc =
		    sa_concat(gcc,
			      "-D__LARGE_C_PREFIX__=int main(int argc, char** argv) {");
	...
	{			/* execute */
		...
		ret =
		    call_proc(child_argv, "could not spawn child process");
	}
